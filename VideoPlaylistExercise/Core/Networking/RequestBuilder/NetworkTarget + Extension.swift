...
struct HttpRequest: RequestBuilder {
  ...
  var pathAppendedURL: URL {
    ...
    if version != .none {
      ...
      // only appendPathComponent if version is not empty because url.appendPathComponent will append "/" even though version is empty
    }
  ...
  func buildURLRequest() -> URLRequest {
    ...
    if let requestBody = self.parameters {
      ...
      if let bodyEncoding = self.bodyEncoding {
        urlRequest.httpBody = self.encodedBody(
          bodyEncoding: bodyEncoding,
